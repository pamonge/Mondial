...
export const SocialMediaComponent = () => {
  ...
  return (
    <div className='flex text-[#141230] gap-10 max-h-11/12 items-center p-8 max-md:flex-col'>
      <div className='min-md:w-1/3 max-sm:w-11/12 max-sm:pt-3 h-auto justify-around'>
        ...
        <div className='flex justify-between py-3.5' >
            {
              socialNetwork.map((net) => (
                <a href={net.to} key={net.key} >
                  <img className='w-7' src={net.img} key={net.key} alt={`${net.key}`} />
                </a>
              ))
            }
        </div>
      ...
      <div className='flex flex-col min-md:justify-center xs:w-full min-lg:w-2/3 h-full md:min-h-[30vh]'>
        ...
        <a href={`mailto:${dataMondial.email}`} className='flex items-center gap-3 md:text-base lg:text-lg'>
          <img className='w-8' src={iconMail} alt="Correo" /> 
          <span className='max-xl:text-sm'>{dataMondial.email}</span>
        </a>
      </div>
    </div>
  )
