const dataAbout = {
    
    intro:['Somos una empresa de gestión inmobiliaria de Murcia, situados en pleno centro de nuestra capital desde 1998, lo que nos avala por nuestra experiencia al tener un profundo conocimiento en este sector como expertos en la zona.',
        'Ofrecemos una atención cualificada, personalizada y profesional, dirigida a asegurar en garantía del cliente con nuestros servicios, una operación transparente, rápida y profesional, comprometiéndonos a que reciban la información necesaria para llevarla a buen fin.'

    ],
    gridItems: ['Ofrecemos una atención cualificada, personalizada y profesional, dirigida a asegurar en garantía del cliente con nuestros servicios, una operación transparente, rápida y profesional, comprometiéndonos a que reciban la información necesaria para llevarla a buen fin.',
     'Garantizamos nuestra actividad profesional mediante un seguro de responsabilidad civil de 1.500.000€.',
     'Utilizamos los mejores y más diversos medios de publicidad, estando presentes en todas las zonas de nuestra ciudad en las primeras posiciones de los más importantes portales inmobiliarios a nivel NACIONAL, siempre en la primera pagina de la sección a la que corresponde su inmueble y con fotos PREMIUM.',
     'Realizamos valoraciones reales con información veraz y actualizada de tu inmueble, basándonos en nuestra extensa experiencia en el sector y utilizando modernos y avanzados sistemas de valoración avalados por el Colegio de Registradores de España, el Ilustre Colegio Notarial de Murcia.'
    
    ],
     conclution:['Cientos de clientes satisfechos hacen honor a nuestro código deontológico de conducta profesional, el cual hemos seguido, seguimos y seguiremos siempre cumpliendo con nuestro lema.'  ]
};

export default dataAbout;